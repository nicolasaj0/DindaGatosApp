  ...
  return (
    <div
      ...
      className={`cursor-pointer rounded-3xl border p-4 shadow-sm transition hover:shadow-md ${
        ...
        isCheckOutToday ? 'border-cyan-300 dark:border-cyan-700 ring-2 ring-cyan-100/50 dark:ring-cyan-950/30 bg-cyan-50/5 dark:bg-cyan-950/10 hover:border-cyan-400' :
        'border-slate-200 dark:border-slate-700/80 hover:border-cyan-300 dark:hover:border-cyan-500 bg-white dark:bg-slate-800'
      }`}
    >
      {isCheckInToday && (
      ...
      {isCheckOutToday && (
        <div className="mb-3 inline-flex items-center gap-1 rounded-full bg-cyan-100 dark:bg-cyan-950 px-2.5 py-0.5 text-[9px] font-bold text-cyan-800 dark:text-cyan-300 uppercase tracking-wider">
          <span className="relative flex h-1.5 w-1.5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-sm">
        ...
        {totalValue !== null && (
          <span 
            className="inline-flex items-center gap-1 rounded-full bg-cyan-50 dark:bg-cyan-950/30 border border-cyan-100 dark:border-cyan-900/40 px-2.5 py-1 text-xs font-bold text-cyan-700 dark:text-cyan-400 ml-auto" 
        ...
        <div className="mt-3 flex flex-col gap-1 text-[10px] text-slate-500 dark:text-slate-400 border-t border-slate-100 dark:border-slate-700 pt-2.5">
          ...
          {hospedagem.dataHoraConfirmacaoCheckOut && (
            <div className="flex items-center gap-1.5 text-cyan-700 dark:text-cyan-400">
              <span className="h-1 w-1 rounded-full bg-cyan-500 flex-shrink-0"></span>
      ...
      {hospedagem.status === 'saindo_hoje' && onCheckOut && (
        <button
          ...
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-600 py-2 text-sm font-semibold text-white shadow-md shadow-cyan-500/10 hover:bg-cyan-500 transition"
        >
          <Check size={16} />
